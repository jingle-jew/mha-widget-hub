...
class MhaControlHub extends HTMLElement{constructor(){super();this.attachShadow({mode:"open"});this._hass=null;this._isEditing=false;this._draggedId="";this._resizeState=null;this._squareUnitFrame=0;this._renderId=0;this._gridScrollCleanup=null;this._screensaverPreview=false;this._screensaverNowBar=true;this._screensaverClockVariant="digital";this._widgets=this._readWidgets()}
set hass(h){this._hass=h;this.render()}get hass(){return this._hass}
connectedCallback(){this.render();this._clockTimer=setInterval(()=>{updateStatusTime(this.shadowRoot);if(this._screensaverPreview)updateScreensaverClock(this.shadowRoot,this._screensaverClockVariant)},1000);this._resizeListener=()=>{if(getLayoutMode(this)==="auto")this.render();else this._scheduleSquareUnitSync()};window.addEventListener("resize",this._resizeListener)}
disconnectedCallback(){clearInterval(this._clockTimer);cancelAnimationFrame(this._squareUnitFrame);this._clearGridScrollListener();window.removeEventListener("resize",this._resizeListener)}
...
}toggleEditMode(){this._isEditing=!this._isEditing;this._syncEditModeDom()}toggleScreensaverPreview(){this._screensaverPreview=!this._screensaverPreview;this.render()}toggleNowBarPreview(){this._screensaverNowBar=!this._screensaverNowBar;this.render()}setScreensaverClockVariant(v="digital"){this._screensaverClockVariant=normalizeClockVariant(v);this.render()}resetGrid(){localStorage.removeItem(ORDER);localStorage.removeItem(SIZES);this._widgets=this._readWidgets();this.render()}
