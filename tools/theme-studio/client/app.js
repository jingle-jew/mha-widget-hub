...
function syncViewport() { const { width, height } = previewViewport(); const frame = $("#preview-frame"); const wrap = $(".preview-wrap"); const scale = Math.min(1, (wrap.clientWidth - 50) / width, (wrap.clientHeight - 50) / height); frame.style.width = `${width * scale}px`; frame.style.height = `${height * scale}px`; const iframe = $("#preview"); iframe.style.width = `${width}px`; iframe.style.height = `${height}px`; iframe.style.transform = `scale(${scale})`; $("#preview-size").textContent = `${width} × ${height}`; }
...
async function createThemeWith(payload) { try { const plan = await api("/api/create-plan", { method: "POST", body: JSON.stringify(payload) }); const files = plan.files.map(file => file.path).join("\n"); if (!confirm(`Plan de création:\n${files}\n\nConfirmer ?`)) return; await api("/api/create-apply", { method: "POST", body: JSON.stringify({ plan }) }); toast("Thème créé. Lancez npm run sync:frontend si nécessaire."); location.reload(); } catch (error) { toast(error.message); } }
