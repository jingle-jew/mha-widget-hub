...
function syncViewport() { const { width, height } = previewViewport(); const frame = $("#preview-frame"); const wrap = $(".preview-wrap"); if (!frame || !wrap || !width || !height) return; const computed = getComputedStyle(wrap); const availableWidth = Math.max(1, wrap.clientWidth - parseFloat(computed.paddingLeft) - parseFloat(computed.paddingRight)); const availableHeight = Math.max(1, wrap.clientHeight - parseFloat(computed.paddingTop) - parseFloat(computed.paddingBottom)); const scale = Math.max(.05, Math.min(1, availableWidth / width, availableHeight / height)); frame.style.width = `${width * scale}px`; frame.style.height = `${height * scale}px`; const iframe = $("#preview"); iframe.style.width = `${width}px`; iframe.style.height = `${height}px`; iframe.style.transform = `scale(${scale})`; $("#preview-size").textContent = `${width} × ${height}`; }
