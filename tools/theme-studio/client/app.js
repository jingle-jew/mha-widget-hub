...
function syncIosPreviewSurface(target, variant) { if (!target || state.themeId !== "ios") return; const tint = variant === "frosted" ? 100 : 0; target.dataset.themeVariant = variant; target.dataset.iosGlass = variant; target.dataset.iosGlassTint = String(tint); target.dataset.iosWidgetTint = tint === 100 ? "tinted" : "transparent"; target.style.setProperty("--mha-ios-glass-tint", String(tint / 100)); target.style.setProperty("--mha-ios-glass-tint-percent", `${tint}%`); target.style.setProperty("--mha-ios-liquid-percent", `${100 - tint}%`); target.style.setProperty("--mha-ios-widget-noise-opacity", tint === 100 ? ".065" : ".072"); target.style.setProperty("--mha-ios-widget-highlight-opacity", tint === 100 ? ".16" : ".56"); }
function syncPreview() { syncViewport(); const iframe = $("#preview"); if (!iframe.contentDocument) return; const doc = iframe.contentDocument; const hub = doc.querySelector("mha-widget-hub"); if (!hub) return; const theme = selectedTheme(); const variant = $("#variant-select").value || theme?.defaultVariant || ""; doc.documentElement.dataset.theme = state.mode; doc.documentElement.dataset.themeStyle = state.themeId; hub.dataset.theme = state.mode; hub.dataset.themeStyle = state.themeId; if (variant) { syncIosPreviewSurface(hub, variant); syncIosPreviewSurface(doc.documentElement, variant); } doc.body.style.background = state.wallpaper || backgroundValue(); doc.querySelector("[data-dev-menu]")?.setAttribute("hidden", ""); localStorage.removeItem("mha-theme-style"); for (const control of state.detail?.controls || []) { if (control.mode !== "shared" && control.mode !== state.mode) continue; const value = currentValue(control); if (value) hub.style.setProperty(control.token, value); } hub.requestRender?.(); setTimeout(() => { syncIosPreviewSurface(hub, variant); syncIosPreviewSurface(doc.documentElement, variant); for (const control of state.detail?.controls || []) { if (control.mode !== "shared" && control.mode !== state.mode) continue; const value = currentValue(control); if (value) hub.style.setProperty(control.token, value); } }, 80); }
