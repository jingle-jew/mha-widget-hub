...
_createWidgetFromCatalogItem(item){
  ...
  const kind=rawKind==="slider"?"slider":rawKind==="button"||rawVariant==="simple-button"?"button":rawKind==="clock"||isClockCatalogVariant(rawVariant)?"clock":"empty";
  const category=item?.category||(kind==="clock"?"utilities":kind==="button"?"actions":"custom");
  const baseSize=kind==="clock"?{w:2,h:2}:kind==="button"?{w:2,h:1}:normalizeWidgetSize(item?.size||{w:2,h:2});
  ...
  return normalizeStoredWidgetContract({
    ...
    kind,
    type:kind,
    component:kind==="clock"?"clock-widget":kind==="slider"?"slider-widget":kind==="button"?"button-widget":"empty-widget",
    category,
    variant:rawVariant,
